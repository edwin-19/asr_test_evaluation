import typer
from nemo.collections.asr.models import EncDecRNNTBPEModel
import torch
from omegaconf import OmegaConf
import json
import re
import os
import soundfile as sf
import itertools as it
from tqdm import tqdm
import werpy
from infer_ctc import write_file
from utils import clean_punctuation_spacing

app = typer.Typer(pretty_exceptions_enable=False)

def read_json(json_path):
    with open(json_path, 'r') as jsonf:
        return [json.loads(f) for f in jsonf.readlines()]

def pad(audios, audio_lens):
    max_len = max(audio_lens)
    padded_audios = []
    for audio, audio_len in zip(audios, audio_lens):
        if audio_len < max_len:
            pad = (0, max_len - audio_len)
            audio = torch.nn.functional.pad(audio, pad)
        padded_audios.append(audio)
    padded_audios = torch.stack(padded_audios)
    return padded_audios, torch.tensor(audio_lens)

def infer(asr_model, audio_array, device):
    audio = torch.tensor(audio_array)
    padded_audios, audio_lens = pad([audio], [audio.shape[0]])
    logits, logits_len = asr_model.forward(input_signal=padded_audios.to(device), input_signal_length=audio_lens.to(device))
    
    outputs = asr_model.decoding.rnnt_decoder_predictions_tensor(
        encoder_output=logits, encoded_lengths=logits_len, return_hypotheses=False
    )
    try:
        preds_text = [outputs[0][0].text]
    except:
        preds_text = [outputs[0].text]
    return preds_text

@app.command()
def main(
    data_path:str=typer.Option("./data/my_data_train/validation.json"),
    model_path:str=typer.Option("./nemo_experiments/conformer-malay-rnnt-averaged.ckpt"),
    config:str=typer.Option("configs/conformer_transducer_bpe.yaml"),
    device:str=typer.Option("cuda"),
    output_dir:str=typer.Option("output/conformer_rnnt"),
    beam_size:int=typer.Option(2), use_beam:bool=typer.Option(False),
):
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
        
    ckpt = torch.load(model_path, weights_only=False, map_location=device)
    if 'state_dict' in ckpt:
        state_dict = ckpt['state_dict']
    else:
        state_dict = ckpt
    
    train_config = OmegaConf.load(config)
    train_config.model.tokenizer.dir = 'tokenizer/tokenizer_spe_bpe_v64_bos_eos'
    
    if use_beam:
        train_config.model.decoding.strategy = 'beam'
        train_config.model.decoding.beam.beam_size = beam_size
    
    asr_model = EncDecRNNTBPEModel(cfg=train_config.model)
    asr_model.load_state_dict(state_dict, strict=True)
    asr_model.to(device)
    asr_model.eval()
    
    manifest = read_json(data_path)
    all_pred = []
    all_gt = []
    for meta in tqdm(manifest):
        new_audio_path = meta['audio_filepath']
        audio, sr = sf.read(new_audio_path)
        with torch.no_grad():
            pred_text = infer(asr_model, audio, device)[0]
            
        # Norm punc
        gt_text = clean_punctuation_spacing(meta['text'])
        pred_text = clean_punctuation_spacing(pred_text)
        
        all_gt.append(gt_text)
        all_pred.append(pred_text)
        
    total_wer = werpy.wer(all_gt, all_pred)
    total_wer = round(total_wer * 100, 4)
    print(f'Total WER: {total_wer}')

    wers = werpy.wers(all_gt, all_pred)
    summary = werpy.summary(all_gt, all_pred)
    summary.to_csv(os.path.join(output_dir, "summary.csv"))
    
    all_data = []
    for gt, pred, wer in zip(all_gt, all_pred, wers):
        all_data.append(f"GT: {gt}\nPred: {pred}\nWER: {wer}\n\n")
        
    all_data.append(f"Total WER: {total_wer}")
    write_file(os.path.join(output_dir, 'results.txt'), all_data)

    
if __name__ == "__main__":
    app()