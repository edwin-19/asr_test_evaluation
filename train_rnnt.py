...
def get_args():
    ...
    parse.add_argument('--conf', type=str, default='configs/conformer_transducer_bpe.yaml', required=False)
...
def main():
    ...
    asr_model = EncDecRNNTBPEModel(cfg=train_config.model, trainer=trainer)

    # Load model
    logging.info("Using Checkpoint: {}".format(train_config.get("encoder_ckpt")))
    ckpt = torch.load(train_config.get("encoder_ckpt", "./mtl_54k.ckpt"), map_location='cuda', weights_only=False)
