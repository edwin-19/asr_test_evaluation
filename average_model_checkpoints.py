...
def main():
    cfg='./configs/conformer_ctc_bpe.yaml'
    # name_prefix, checkpoint_paths, save_ckpt_only = process_config(cfg)
    name_prefix = "conformer-malay"
    # checkpoint_sels = natsorted(list(glob.glob('/wiz-llm-storage/edwin/code/my_conformer/exp/my_da/2024-10-21_09-47-54/checkpoints/*.ckpt')))
    checkpoint_sels = natsorted(list(glob.glob('/dataset/asr/my_train/nemo_experiments/Conformer-CTC-BPE-Malay/2026-04-30_02-39-22/checkpoints/*.ckpt')))
    checkpoint_sels = [path for path in checkpoint_sels if 'last' not in str(path) and 'best' not in str(path)]

    # extra_ckpt = natsorted(list(glob.glob("/wiz-llm-storage/edwin/code/my_conformer/exp/maxis_da/2024-09-28_04-07-25/checkpoints/*.ckpt")))
    # extra_ckpt = [path for path in extra_ckpt if 'last' not in str(path) and 'best' not in str(path)]

    # checkpoint_paths = checkpoint_sels[:1] + [checkpoint_sels[-1]] + [checkpoint_sels[1]] + [checkpoint_sels[2]]
    checkpoint_paths = checkpoint_sels[:5]
    save_ckpt_only = True
    if not save_ckpt_only:
        checkpoint_callback = ModelCheckpoint(monitor="val_wer", dirpath='checkpoints/', save_top_k=15)
        trainer = pl.Trainer(gpus=2, max_epochs=200, log_every_n_steps=2000, callbacks=checkpoint_callback,
                     accelerator='cuda', strategy=DDPStrategy(find_unused_parameters=False), accumulate_grad_batches=16)
